...
int main(int argc, char **argv)
{
    ...
    if (debug==0) for (i=0;i<1;i++)       printf("%d: BEFORE addrVec2[%d][%d] = %f\n",me,me,i,addrVec2[me][i]);
    if (debug==1) for (i=0;i<bufSize;i++) printf("%d: BEFORE addrVec2[%d][%d] = %f\n",me,me,i,addrVec2[me][i]);
    fflush(stdout);
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    for (j=0;j<nproc;j++){
        target = (me+j) % nproc;
        if (debug==1) { printf("%d: ARMCI_Put fired to %d\n",me,target); fflush(stdout); }
        status = ARMCI_Put(b1, addrVec2[target], bufSize*sizeof(double), target); assert(status==0);
    }
    t1 = MPI_Wtime();
    ARMCI_AllFence();
    t2 = MPI_Wtime();
    MPI_Barrier(MPI_COMM_WORLD);
    t3 = MPI_Wtime();
    printf("%d: TIME ARMCI_Put = %f s ARMCI_AllFence = %f s MPI_Barrier = %f s\n",me,t1-t0,t2-t1,t3-t2);
    fflush(stdout);
    MPI_Barrier(MPI_COMM_WORLD);
