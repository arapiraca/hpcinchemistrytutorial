...
int matmul2(int rank, long blksz)
{
    ...
    g_d1  = GA_Duplicate(g_a,"matrix D1");
    if(g_d1 == 0){
        if (me == 0) printf("%s: GA_Duplicate failed at line %d\n",__FILE__,__LINE__);
    };

    g_d2  = GA_Duplicate(g_a,"matrix D2");
    if(g_d2 == 0){
        if (me == 0) printf("%s: GA_Duplicate failed at line %d\n",__FILE__,__LINE__);
    };
...
// //             p_d[i+j*blksz] *= beta;
//             for (k=0;k<blksz;k++ ){
//                 p_d[i+j*blksz]+=alpha*p_b[i+k*blksz]*p_a[k+j*blksz];
//             }
//         }
//     }
// // // //     for (k=0;k<(blksz*blksz);k++) p_d[k]=0.0;
...
#endif
// #else
//     GA_Add(&alpha,g_c,&beta,g_d1,g_error);
// #ifdef DEBUG
//     GA_Print(g_c);
//     GA_Print(g_d1);
// #endif
